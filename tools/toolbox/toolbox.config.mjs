  ...
  tools: [
    ...
    },
    {
      ...
      fields: [
        ...
        { name: 'stage', flag: '--stage', type: 'select', options: ['complete', 'registration', 'closed', 'midplay'], default: 'complete', help: 'stop mid-lifecycle and leave the tournament for app inspection: registration = open w/ a pending request; closed = past the registration deadline (verifies RLS blocks late joins); midplay = half of round 1 scored (verifies no premature advancement)', showIf: { scenario: ['tournament'] } },
        { name: 'auto-rounds', flag: '--auto-rounds', type: 'checkbox', default: true, help: 'run the WHOLE tournament round-by-round to completion — playoffs generated, every step invariant-checked, failures drafted into simulations/reports/<name>.md (ignored when stage != complete)', showIf: { scenario: ['tournament'] } },
        { name: 'league-attach', flag: '--league-attach', type: 'checkbox', help: 'attach the tournament to the [SIM] Toolbox League — verifies league-vs-global PLUPR weighting and locks a season period after completion', showIf: { scenario: ['tournament'] } },
