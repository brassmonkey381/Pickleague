-- ============================================================
-- Consolidate preview_tournament_payout
--
-- Two prior migrations each defined preview_tournament_payout with
-- only a subset of the branches:
--   * migration_universal_tournament_payout.sql — MLP delegate +
--     single/double_elim (using wrong column names: player1_id /
--     partner1_id instead of team1_player1 / team1_player2).
--   * migration_non_elim_payout_and_autoclose.sql — MLP delegate +
--     round_robin / pool_play / rotating_partners.
-- The function is `create or replace`d in both, so whichever loads
-- last in Supabase wins and the other branch's behaviour disappears.
--
-- This migration installs the FINAL authoritative version that
-- includes every branch:
--   * mlp / mlp_random            → delegate to preview_mlp_tournament_payout
--   * single_elim / double_elim   → finals/grand_final winner & runner-up
--   * round_robin / pool_play     → standings by wins, point diff, rating
--   * rotating_partners           → individual wins across rotating rounds
--
-- The elim branch is rewritten with the correct tournament_matches
-- column names (team1_player1 / team1_player2 / team2_player1 / team2_player2).
--
-- Run AFTER:
--   migration_universal_tournament_payout.sql
--   migration_non_elim_payout_and_autoclose.sql
-- ============================================================
