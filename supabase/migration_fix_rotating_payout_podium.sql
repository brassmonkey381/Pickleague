-- Fix: rotating-partners payout crowned the wrong champion.
--
-- preview_tournament_payout's rotating branch re-derived standings with its
-- own tiebreakers (wins desc, point_diff desc, rating) while
-- compute_tournament_final_ranks — the stored standings that settle wagers
-- and render in the app — orders by (wins desc, losses asc, rating). When
-- wins tied, the payout + champion badge went to a DIFFERENT player than the
-- final standings' #1 (observed live via the toolbox economy run).
-- The rotating branch now reads tournament_final_ranks when populated
-- (always, post-completion) so podium == stored standings; local stats
-- remain only as a fallback. Body otherwise the committed/prod version.
-- Known follow-up: RR/pool branches still re-derive (point_diff tiebreak) —
-- they agree with final ranks except on exact win-ties; unify later.
