-- Fix: elimination formats never paid 3rd place — the share was silently
-- stranded in the pot whenever the payout structure had 3+ slots (default
-- 60/25/15). Double elim now pays the loser of the last losers-bracket match
-- as an unambiguous #3; single elim splits the 3rd share between the two
-- semifinal losers (a standard tie-for-3rd). Found by the toolbox economy
-- checks (7-player double elim: expected 3 payout notifications, got 2).
-- Includes the earlier rotating-podium fix; body otherwise prod verbatim.
  ...
  if v_format in ('single_elimination', 'double_elimination') then
    ...
    if v_slots >= 3 then
      if v_format = 'double_elimination' then
        select tm.player1_id, tm.partner1_id, tm.player2_id, tm.partner2_id, tm.winner_team
          into v_third
          from public.tournament_matches tm
          join public.tournament_rounds tr on tr.id = tm.round_id
         where tr.tournament_id = p_tournament_id
          ...
          return query select
            ...
            (select string_agg(p.full_name, ' & ' order by p.full_name) from public.profiles p
              where p.id = any(case when v_third.winner_team = 'team1'
                                    then array_remove(array[v_third.player2_id, v_third.partner2_id], null)
                                    else array_remove(array[v_third.player1_id, v_third.partner1_id], null) end)),
            ...
            floor(floor(v_pool * v_structure[3] / 100.0)
                  / greatest(case when v_third.winner_team = 'team1'
                                  then coalesce(array_length(array_remove(array[v_third.player2_id, v_third.partner2_id], null), 1), 0)
                                  else coalesce(array_length(array_remove(array[v_third.player1_id, v_third.partner1_id], null), 1), 0) end, 1))::int,
            0.100::numeric(6,3);
        end if;
      else
            ...
            select 3, null::uuid,
              ...
              from (
                select case when tm.winner_team = 'team1'
                            then array_remove(array[tm.player2_id, tm.partner2_id], null)
                            else array_remove(array[tm.player1_id, tm.partner1_id], null) end as uids
                  from public.tournament_matches tm
                  join public.tournament_rounds tr on tr.id = tm.round_id
                  join public.tournament_rounds fin on fin.tournament_id = tr.tournament_id and fin.round_type = 'finals'
                 where tr.tournament_id = p_tournament_id
